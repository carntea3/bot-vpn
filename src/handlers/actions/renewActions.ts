...
async function handleDirectAccountRenewal(ctx) {
    ...
    try {
        ...
        const server = await dbGetAsync('SELECT * FROM Server WHERE nama_server = ? OR domain = ?', [account.server, account.server]);
        if (!server) {
            // Log for debugging
      ...
      [
        ...
        Markup.button.callback('2 Hari', `duration_renew_${account.protocol}_${server.id}_2`),
		Markup.button.callback('3 Hari', `duration_renew_${account.protocol}_${server.id}_3`),
		Markup.button.callback('4 Hari', `duration_renew_${account.protocol}_${server.id}_4`),
		Markup.button.callback('5 Hari', `duration_renew_${account.protocol}_${server.id}_5`)
      ],
      [
        Markup.button.callback('6 Hari', `duration_renew_${account.protocol}_${server.id}_6`),
        Markup.button.callback('7 Hari', `duration_renew_${account.protocol}_${server.id}_7`),
		Markup.button.callback('8 Hari', `duration_renew_${account.protocol}_${server.id}_8`),
		Markup.button.callback('9 Hari', `duration_renew_${account.protocol}_${server.id}_9`),
		Markup.button.callback('10 Hari', `duration_renew_${account.protocol}_${server.id}_10`)
      ],
	 [
        Markup.button.callback('15 Hari', `duration_renew_${account.protocol}_${server.id}_15`),
        Markup.button.callback('20 Hari', `duration_renew_${account.protocol}_${server.id}_20`),
		Markup.button.callback('25 Hari', `duration_renew_${account.protocol}_${server.id}_25`),
		Markup.button.callback('30 Hari', `duration_renew_${account.protocol}_${server.id}_30`),
		Markup.button.callback('40 Hari', `duration_renew_${account.protocol}_${server.id}_40`)
      ],
	  [
        Markup.button.callback('50 Hari', `duration_renew_${account.protocol}_${server.id}_50`),
        Markup.button.callback('60 Hari', `duration_renew_${account.protocol}_${server.id}_60`),
		Markup.button.callback('70 Hari', `duration_renew_${account.protocol}_${server.id}_70`),
		Markup.button.callback('80 Hari', `duration_renew_${account.protocol}_${server.id}_80`),
		Markup.button.callback('90 Hari', `duration_renew_${account.protocol}_${server.id}_90`)
      ],
            [
...
async function handlePaymentConfirmation(ctx, action) {
    ...
    try {
        ...
        const accountExists = await dbGetAsync('SELECT * FROM akun_aktif WHERE username = ? AND jenis = ?', [username, protocolLower]);
        if (!accountExists) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout: Koneksi ke server terlalu lama')), 60000));
            ...
            // Refund balance
            await dbRunAsync('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [totalHarga, userId]);
            return ctx.reply(`❌ *Gagal memperpanjang akun*\n\n` +
                `Detail error: ${handlerError.message}\n\n` +
                `Saldo Anda telah dikembalikan.`, { parse_mode: 'Markdown' });
        }
        // Validate response
        if (!msg || typeof msg !== 'string') {
            ...
            // Refund balance
            await dbRunAsync('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [totalHarga, userId]);
            return ctx.reply('❌ *Terjadi kesalahan saat memperpanjang akun.*\n\n' +
                'Response dari server tidak valid. Saldo Anda telah dikembalikan.', { parse_mode: 'Markdown' });
        }
        ...
        try {
            ...
            if (newExpiredAt) {
                ...
                const existingAccount = await dbGetAsync(`SELECT id, username, protocol, server, expired_at FROM accounts WHERE username = ? AND protocol = ?`, [username, protocolLower]);
                ...
                if (protocolLower === '3in1') {
                    ...
                    await dbRunAsync(`UPDATE accounts SET expired_at = ?, status = 'active', expiry_warning_3d_sent = 0, expiry_warning_1d_sent = 0, expired_notified = 0
             WHERE username = ? AND UPPER(protocol) IN ('VMESS', 'VLESS', 'TROJAN')`, [newExpiredAt, username]);
                    logger.info(`✅ Updated expired_at for 3in1 account (vmess, vless, trojan)`);
                }
                else {
                    // Update using case-insensitive protocol match
                    const result = await dbRunAsync(`UPDATE accounts SET expired_at = ?, status = 'active', expiry_warning_3d_sent = 0, expiry_warning_1d_sent = 0, expired_notified = 0
             WHERE username = ? AND UPPER(protocol) = UPPER(?)`, [newExpiredAt, username, protocolLower]);
  ...
  role: user.role
        ...
        try {
            await ctx.reply('❌ *Terjadi kesalahan saat memproses perpanjangan.*\n\n' +
                `Detail: ${error.message}\n\n` +
                'Silakan hubungi admin jika masalah berlanjut.', { parse_mode: 'Markdown' });
        }
        catch (replyError) {
...
async function handleRenewAccount(ctx, protocol, serverId, renewFunction) {
    ...
    try {
        ...
        await ctx.telegram.sendMessage(chatId, `📝 Perpanjang Akun ${protocol.toUpperCase()}\n\nMasukkan username yang ingin diperpanjang:`);
    }
    catch (error) {
