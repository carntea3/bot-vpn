    ...
    bot.action('admin_view_topup', async (ctx) => {
      ...
      `);
            ...
            rows.forEach((row, i) => {
                ...
                const waktu = escapeMarkdown(new Date(row.created_at).toLocaleString('id-ID'));
...
function registerAdminListResellersAction(bot) {
  bot.action('admin_listreseller', async (ctx) => {
    ...
    try {
      ...
      logger.error('❌ Gagal ambil list reseller:', err);
...
function registerAdminListUsersAction(bot) {
  bot.action('admin_listuser', async (ctx) => {
    ...
    try {
      const rows = await new Promise((resolve, reject) => {
        global.db.all(
          'SELECT user_id, username, role, saldo FROM users LIMIT 20',
          (err, rows) => {
            if (err) reject(err);
            else resolve(rows);
          }
      ...
      const text = `👥 *List Pengguna \\(max 20\\):*\n\n${list}`;

      await ctx.reply(text, { parse_mode: 'MarkdownV2' });

    } catch (err) {
      logger.error('❌ Gagal ambil list user:', err);
...
function registerAdminRestoreDBAction(bot) {
    bot.action('admin_restore2_db', async (ctx) => {
        ...
        await ctx.reply('📤 *Silakan kirim file backup database (.db) yang ingin direstore.*\n' +
            '_Contoh: botvpn_2025-06-01_10-00.db_', { parse_mode: 'Markdown' });
    });
}
/**
...
function registerApproveDepositAction(bot) {
    bot.action(/^approve_deposit_(.+)$/, async (ctx) => {
        ...
        try {
            ...
            // Log to topup_log
            await dbRunAsync(`INSERT INTO topup_log (user_id, username, amount, reference, created_at)
         VALUES (?, ?, ?, ?, datetime('now'))`, [deposit.user_id, depositUser.username || depositUser.first_name, deposit.amount, invoiceId]);
            // Notify user
            const mention = depositUser.username ? `@${depositUser.username}` : depositUser.first_name;
            await ctx.telegram.sendMessage(deposit.user_id, `✅ *Deposit Disetujui!*\n\n` +
                `💰 Jumlah: Rp ${deposit.amount.toLocaleString('id-ID')}\n` +
                `💳 Saldo baru: Rp ${newBalance.toLocaleString('id-ID')}\n` +
                `🆔 Invoice: \`${invoiceId}\`\n\n` +
                `Terima kasih! Saldo Anda telah ditambahkan.`, { parse_mode: 'Markdown' });
            ...
            await ctx.editMessageText(`✅ *Deposit Berhasil Disetujui*\n\n` +
    ...
    bot.action(/^reject_deposit_(.+)$/, async (ctx) => {
        ...
        try {
            ...
            await ctx.telegram.sendMessage(deposit.user_id, `❌ *Deposit Ditolak*\n\n` +
                ...
                `Silakan upload ulang dengan bukti pembayaran yang sesuai.`, { parse_mode: 'Markdown' });
            await ctx.editMessageText(`❌ *Deposit Ditolak*\n\n` +
