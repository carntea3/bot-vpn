...
async function deleteVless(username: string, serverId: number): Promise<string> {
    console.log(`🗑️ Deleting VLESS account: ${username}`);
    ...
    return new Promise((resolve) => {
        db.get('SELECT * FROM Server WHERE id = ?', [serverId], async (err, server) => {
            ...
            console.log(`📡 Connecting to ${server.domain} for VLESS deletion...`);
...
`;
                ...
                console.log('🔨 Executing VLESS delete command...');
                ...
                conn.exec(cmd, (err, stream) => {
                    ...
                    stream.on('close', (code, signal) => {
                        ...
                        if (code !== 0) {
                            ...
                            return resolve('❌ Gagal menghapus akun VLESS di server.');
                        }

                        if (!output.includes('SUCCESS')) {
                            return resolve('❌ Gagal menghapus akun VLESS.');
                        }

                        console.log('✅ VLESS deleted from server:', username);
                        resolve('SUCCESS');
                    })
                        .on('data', (data) => {
